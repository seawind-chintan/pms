...
foreach ($rooms_for_rack as $room_key => $room_data) {
	...
	<div class="col-md-3 col-sm-6 col-xs-12">
          <?php if($room_data->room_status_id === 1) { ?> <a href="<?php echo $this->Url->build([
    "controller" => "checkins",
    "action" => "add",
    "?" => ["room_id" => $room_data->id, "property_id" => $room_data->property_id],
    //"#" => "first",
]); ?>"> <?php } ?>
          ...
          <?php if($room_data->room_status_id) { ?> </a> <?php } ?>
