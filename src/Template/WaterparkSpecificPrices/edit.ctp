    ...
    <div class="col-md-12">
      ...
      <div class="box box-primary">
        ...
        <?= $this->Form->create($waterparkSpecificPrice, array('role' => 'form')) ?>
          <div class="box-body">
          <?php
            //pr($waterparkSpecificPrice->single_date);
            //var_dump($this->Time->format($waterparkSpecificPrice->single_date, 'yyyy-MM-dd'));exit;
            echo $this->Form->input('property_id', ['options' => $properties]);
            echo $this->Form->input('type', ['options' => ['Single Day', 'Date Range']]);
            
            echo $this->Form->input('single_date', ['empty' => true, 'value' => $this->Time->format($waterparkSpecificPrice->single_date, 'yyyy-MM-dd'), 'class' => 'datepicker form-control', 'type' => 'text', 'templates' => ['inputContainer' => '<div class="row" id="single_date_selector"><div class="input {{type}}{{required}} col-md-12">{{content}}</div></div>', 'inputContainerError' => '<div class="row"><div class="input {{type}}{{required}} error col-md-12">{{content}}{{error}}</div></div>']]);
            
            echo $this->Form->input('from_date', ['empty' => true, 'default' => '', 'value' => $this->Time->format($waterparkSpecificPrice->from_date, 'yyyy-MM-dd'), 'class' => 'datepicker form-control', 'type' => 'text', 'templates' => ['inputContainer' => '<div class="row" id="date_range_selector"><div class="input {{type}}{{required}} col-md-6">{{content}}</div>', 'inputContainerError' => '<div class="row"><div class="input {{type}}{{required}} error col-md-6">{{content}}{{error}}</div>']]);

            echo $this->Form->input('to_date', ['empty' => true, 'default' => '', 'value' => $this->Time->format($waterparkSpecificPrice->to_date, 'yyyy-MM-dd'), 'class' => 'datepicker form-control', 'type' => 'text', 'templates' => ['inputContainer' => '<div class="input {{type}}{{required}} col-md-6">{{content}}</div></div>', 'inputContainerError' => '<div class="input {{type}}{{required}} error col-md-6">{{content}}{{error}}</div></div>']]);
